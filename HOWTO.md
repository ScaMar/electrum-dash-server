How to run your own Electrum Deviant server
========================================
...
Electrum Deviant server on Linux. It is structured as a series of steps you need to
...
document, we assume that user is called 'deviant'). We also assume the
deviant user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run deviantd and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit ddeviantd 
...
only leaving spare cycles for deviantd. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running deviantd and Electrum Deviant server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `deviantd` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser deviant --disabled-password
    $ sudo apt-get install git
    $ sudo su - deviant
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/deviant/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download deviantd

Recommend downloading latest version directly from github/Deviantcoin

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev
    $ sudo su - deviant
    $ cd ~/src && wget https://github.com/Deviantcoin/Source/releases/download/3.0.0.1-8e06493/dev-3.0.0.1-linux-x86_64.zip
    $ unzip dev-3.0.0.1-linux-x86_64.zip
    $ cd dev-3.0.0.1-linux-x86_64.0/bin
    $ cp -a deviantd deviant-cli deviant-tx ~/bin

### Step 3. Configure and start deviantd

In order to allow Electrum Deviant to "talk" to `deviantd`, we need to set up an RPC
username and password for `deviantd`. We will then start `deviantd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.DeviantCore
    $ $EDITOR ~/.DeviantCore/deviant.conf

Write this in `deviant.conf`:
...
If you have an existing installation of deviantd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ deviantd -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `deviantd`:

    $ deviantd

Allow some time to pass for `deviantd` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ deviant-cli getblockchaininfo

Before starting the Electrum Deviant server your deviantd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start deviantd at boot
time, running as the 'deviant' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum Deviant server

We will download the latest git snapshot for Electrum to configure and install it:

    $ cd ~
    $ git clone https://github.com/Deviantcoin/electrum-deviant-server.git
    $ cd electrum-deviant-server
...
### Optional Step 5: Install Electrum Deviant dependencies manually

Electrum Deviant server depends on various standard Python libraries and leveldb. These will usually be
...
Electrum Deviant server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum Deviant full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum Deviant server config.
...
### Step 9. Configure Electrum Deviant server

Electrum Deviant reads a config file (/etc/electrum-deviant.conf) when starting up. This
file includes the database setup, deviantd RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-deviant.conf
...
Electrum Deviant server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-deviant-server.
If you're using the user `deviant` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "deviant hard nofile 65536" >> /etc/security/limits.conf
     echo "deviant soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum Deviant server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u deviant -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close deviantd for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting deviantd (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum Deviant server

The magic moment has come: you can now start your Electrum Deviant server as root (it will su to your unprivileged user):
...
You should see this in the log file:
    ...
    starting Electrum Deviant server
...
If you want to stop Electrum Deviant server, use the 'stop' command:
...
### Step 12. Test the Electrum Deviant server
...
current server and connect to your new Electrum Deviant server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
deviants to confirm that everything is working properly.
...
If you're operating a public Electrum Deviant server please subscribe
